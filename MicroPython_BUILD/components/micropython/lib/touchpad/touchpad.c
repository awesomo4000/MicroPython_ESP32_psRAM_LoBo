...
//#define TOUCHPAD_BASELINE_UPDATE_COUNT_THRESHOLD    1200     /**< 800ms; Baseline update cycle. */
...
//#define TOUCHPAD_HYSTERESIS_THRESHOLD_PERCENT       0.01    /**< 10%; The threshold prevents frequent triggering. */
...
/* Expose internal touchpad state */
esp_err_t iot_tp_get_status(const tp_handle_t tp_handle, tp_status_t *tp_status)
{
    POINT_ASSERT(TAG, tp_handle);
    POINT_ASSERT(TAG, tp_status);
    tp_dev_t *tp_dev = (tp_dev_t *) tp_handle;
    *tp_status = tp_dev->state;
    return ESP_OK;
}
